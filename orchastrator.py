...
async def handle_query(file: UploadFile = File(...)) -> dict:
    try:
        ...
        if intent == "price":
            ...
            md = requests.get(f"http://localhost:8002/market-data?ticker={ticker}", timeout=20).json()
            if "error" in md:
        ...
        if intent == "filings":
            ...
            filings = requests.get(f"http://localhost:8003/filings?ticker={ticker}", timeout=20) \
                              .json().get("filings", [])
            if not filings:
        ...
        if intent in ["historical", "qa"]:
            rv = requests.post("http://localhost:8004/retrieve", json={"query": text}, timeout=20)
            rv.raise_for_status()
            data = rv.json()
            ...
            an = requests.post("http://localhost:8005/analyze", json=analyze_payload, timeout=20)
            an.raise_for_status()
            gen = requests.post("http://localhost:8006/generate", json=an.json(), timeout=20)
            gen.raise_for_status()
            return {"response": gen.json().get("text", "")}

        # 6) PORTFOLIO with generalized clarification
        if intent == "portfolio":
            allocs = requests.get("http://localhost:8007/portfolio-allocation", timeout=20).json()
            ...
            return pipeline({"data": [m, f]}, [
                "http://localhost:8005/analyze", "http://localhost:8006/generate"
            ])

        # 7) DEFAULT QA-only
        return pipeline({"query": text}, ["http://localhost:8006/generate"])
